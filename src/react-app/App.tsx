...
function generateStructuredVow(data: string[]): {
  ...
  const image = feelNoun[partnerLang];
  ...
  return {
    text: [
      ...
      ["💬 О нём", <>
        ...
        как моё сердце становится <u>{image}</u> для тебя.
      </>],
      ["🫶 О себе", <>
        Из своей <i>{q3}</i> я дарю тебе любовь через <i>{c3}</i>, потому что именно так <u>{soul}</u> моя душа рядом с тобой.
      </>],
      ["🤝 Обещание", <>
        И я обещаю <i>{verb}</i> <u>{space}</u>, где наши чувства будут расти сильнее каждого дня.
      </>],
