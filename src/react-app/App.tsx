...
const seeingVerb: Record<string, string> = {
  "Безопасность и забота": "замечаю",
  "Притяжение и страсть": "вижу",
  "Поддержка и признание": "наблюдаю",
  "Забота и принятие": "чувствую",
  "Общие смыслы и видение": "считываю",
  "Вдохновение и интуиция": "ощущаю",
  "Тишина и слияние": "вслушиваюсь"
};

const feelNoun: Record<string, string> = {
  "Безопасность и забота": "тепло",
  "Притяжение и страсть": "огонь",
  "Поддержка и признание": "силу",
  "Забота и принятие": "нежность",
  "Общие смыслы и видение": "замысел",
  "Вдохновение и интуиция": "свет",
  "Тишина и слияние": "тишину"
};

const soulVerb: Record<string, string> = {
  "Безопасность и забота": "покоится",
  "Притяжение и страсть": "вспыхивает",
  "Поддержка и признание": "укрепляется",
  "Забота и принятие": "мягко раскрывается",
  "Общие смыслы и видение": "отражается",
  "Вдохновение и интуиция": "поёт",
  "Тишина и слияние": "затихает"
};

const buildVerb: Record<string, string> = {
  "Безопасность и забота": "сохранять",
  "Притяжение и страсть": "разжигать",
  "Поддержка и признание": "укреплять",
  "Забота и принятие": "оберегать",
  "Общие смыслы и видение": "направлять",
  "Вдохновение и интуиция": "вдохновлять",
  "Тишина и слияние": "собирать в тишине"
};

const spaceImage: Record<string, string> = {
  "Безопасность и забота": "остров спокойствия",
  "Притяжение и страсть": "вспышка чувств",
  "Поддержка и признание": "надёжная опора",
  "Забота и принятие": "уютное пространство",
  "Общие смыслы и видение": "пространство замысла",
  "Вдохновение и интуиция": "звучащее пространство",
  "Тишина и слияние": "молчаливое присутствие"
};
...
const imagePhrase: Record<string, string> = {
  "Безопасность и забота": "домом, где спокойно",
  "Притяжение и страсть": "огнём, который зовёт",
  "Поддержка и признание": "опорой, в которую верят",
  "Забота и принятие": "теплом, к которому хочется возвращаться",
  "Общие смыслы и видение": "местом, где оживают смыслы",
  "Вдохновение и интуиция": "пространством, где рождаются идеи",
  "Тишина и слияние": "тишиной, где всё сливается в одно"
};
...
function generateStructuredVow(data: string[]): {
  ...
  const image = feelNoun[partnerFeel];
  ...
  return {
    text: [
      ...
      ["💬 О нём", <>
        Я <u>{see}</u> в тебе <i>{q2}</i>, и через твои <i>{c2}</i> чувствую,
        как моё сердце становится <u>{image}</u> — {imageDetail}.
      </>],
      ["🫶 О себе", <>
        Из своей <i>{q3}</i> я дарю тебе любовь через <i>{c3}</i>, потому что именно так <u>{soul}</u> моя душа рядом с тобой.
      </>],
      ["🤝 Обещание", <>
        И я обещаю <i>{verb}</i> <u>{space}</u>, где наши чувства будут <u>{grow}</u> с каждым днём.
      </>],
